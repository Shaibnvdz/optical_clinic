...
if (isset($_GET["id"])) {
    ...
    if ($stmt_fetch_appointment->fetch()) {
        ...
        $update_query = "UPDATE appointments SET status = 'approved' WHERE id = ?";
        ...
        if ($update_stmt->execute()) {
            ...
            try {
                ...
                $mail->Body = 'Your appointment on ' . $date . ' at ' . $time . ' has been approved.';

                $mail->send();

                echo "<script> alert('Appointment approved successfully. Email sent to user.'), window.location.href='appointment-management.php' </script>";
                exit;
            } catch (Exception $e) {
